...
(function () {
  ...
  function hydrate(opts) {
    ...
    return _minWithConfidence().then(function (res) {
      ...
      if (window.ST) { ST.txns = (window.preservePendingTxns ? window.preservePendingTxns(core.txns || []) : (core.txns || [])); if (!(window.AI && AI.sync && AI.sync.isUnsent('recurring'))) ST.recurring = core.recurring || []; if (core.budgets) { var _ct = ST.budgets && ST.budgets.catTargets, _lk = ST.budgets && ST.budgets.locked; ST.budgets = core.budgets; if (!ST.budgets.catTargets && _ct) ST.budgets.catTargets = _ct; if (!ST.budgets.locked && _lk) ST.budgets.locked = _lk; /* -451 */ } if (core.provPaid && typeof core.provPaid === 'object') ST.provPaid = core.provPaid; /* -449: targets were wiped by this replace; provisional payment now comes from the cloud */ if (core.batches && !(window.AI && AI.sync && AI.sync.isUnsent('imports'))) ST.importBatches = core.batches;
