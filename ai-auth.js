...
(function () {
  ...
  function _sessionUid(){
    /* This app's own session first. Both apps live on one host, so localStorage holds a
       ...
       gone on the next load, with the status green (-412). */
    try{
  ...
  function hydrate(opts) {
    ...
    return _minWithConfidence().then(function (res) {
      ...
      if (window.ST) { ST.txns = (window.preservePendingTxns ? window.preservePendingTxns(core.txns || []) : (core.txns || [])); if (!(window.AI && AI.sync && AI.sync.isUnsent('recurring'))) ST.recurring = core.recurring || []; if (core.budgets) { var _ct = ST.budgets && ST.budgets.catTargets, _lk = ST.budgets && ST.budgets.locked; ST.budgets = core.budgets; if (!ST.budgets.catTargets && _ct) ST.budgets.catTargets = _ct; if (!ST.budgets.locked && _lk) ST.budgets.locked = _lk; /* -451 */ } if (core.provPaid && typeof core.provPaid === 'object') ST.provPaid = core.provPaid; if (core.taxPaid && typeof core.taxPaid === 'object') ST.taxPaid = core.taxPaid /* -457 */; /* -449: targets were wiped by this replace; provisional payment now comes from the cloud */ if (core.batches && !(window.AI && AI.sync && AI.sync.isUnsent('imports'))) ST.importBatches = core.batches;
      ...
      return Promise.all([
        ...
        try { if (window.ST_LS && ls && !_unsent('livestock')) { var _lsHas = ((ls.herds&&ls.herds.length)||(ls.camps&&ls.camps.length)||(ls.animals&&ls.animals.length)); var _lsLoc = ((ST_LS.herd&&ST_LS.herd.length)||(ST_LS.camps&&ST_LS.camps.length)); if (_lsHas || !_lsLoc) { ST_LS.camps = ls.camps || []; ST_LS.herd = ls.herds || []; if (ls.benchmarks) ST_LS.benchmarks = ls.benchmarks; ST_LS.moves = ls.moves || []; ST_LS.treatments = ls.treatments || []; ST_LS.animals = ls.animals || []; ST_LS.health = ls.health || []; ST_LS.breedings = ls.breedings || []; /* Herds whose stored count did not yet include a move recorded elsewhere - the phone at a gate, or another desktop. load.livestock has already folded those moves in, so the figure on screen is the one the ledger supports; this is what the farmer gets told about it, on the Dashboard. */ ST_LS._stockRecon = (ls.stockRecon && ls.stockRecon.length) ? ls.stockRecon : null; } } } catch (e) { console.error('livestock apply', e); }
        try { if (window.ST_CROP && cr && !_unsent('crops')) { var _crHas = ((cr.lands&&cr.lands.length)||(cr.events&&cr.events.length)||(cr.inputs&&cr.inputs.length)); var _crLoc = ((ST_CROP.lands&&ST_CROP.lands.length)||(ST_CROP.events&&ST_CROP.events.length)); if (_crHas || !_crLoc) { ST_CROP.lands = cr.lands || []; ST_CROP.events = cr.events || []; ST_CROP.inputs = cr.inputs || []; ST_CROP.removed = cr.removed || []; if (cr.season) ST_CROP.season = cr.season; if (cr.compliance) ST_CROP.compliance = cr.compliance; } } } catch (e) { console.error('crops apply', e); }
        try { if (window.ST_FRUIT && orc && !_unsent('orchard')) { var _orcHas = (orc.blocks && orc.blocks.length); var _locHas = (ST_FRUIT.blocks && ST_FRUIT.blocks.length); if (_orcHas || !_locHas) { ST_FRUIT.blocks = orc.blocks || []; ST_FRUIT.pricing = orc.pricing || {}; ST_FRUIT.sprayDiary = orc.sprayDiary || {}; ST_FRUIT.removedSprays = orc.removedSprays || []; ST_FRUIT.harvest = orc.harvest || []; if (orc.comply) ST_FRUIT.comply = orc.comply; try{ if(window.orComplyEnsure) orComplyEnsure(); }catch(_){} if (orc.market) ST_FRUIT.market = orc.market; if (typeof window.orRebuildPhi === 'function') { try { window.orRebuildPhi(); } catch (_) {} } } } } catch (e) { console.error('orchard apply', e); }
        ...
        /* The same farm facts live in TWO objects and only one of them round-trips.
           ...
           VAT 201 summary, the registration nudge). Since clearAllToFresh() sets
           FARM.vat = false and runs on any account/farm mismatch, a farmer signing in on
           a second device had VAT silently switched OFF however they answered, and the
           statutory documents printed a blank VAT and tax number. Copy the profile into
           FARM as well, so the server stays the single source of truth for both.
           Note the != null tests: `false` is a real answer and must survive them. */
        try {
          if (window.ST && pf && !_unsent('settings')) {
            ...
            if (window.FARM) {
              ...
              if (pf.farmHa     != null) FARM.ha    = Number(pf.farmHa) || 0;
              if (pf.vatRegistered != null) FARM.vat = !!pf.vatRegistered;
              if (pf.vatNumber  != null) FARM.vatNo = pf.vatNumber;
              if (pf.taxNumber  != null) FARM.taxNo = pf.taxNumber;
            }
          }
        ...
        try { if (window.ST_FUEL && Array.isArray(fu) && !_unsent('fuel') && (fu.length || !(ST_FUEL.issues && ST_FUEL.issues.length))) ST_FUEL.issues = fu; } catch (e) { console.error('fuel apply', e); }
        try { if (window.ST && Array.isArray(dc) && !_unsent('documents') && (dc.length || !(ST.docs && ST.docs.length))) ST.docs = dc; if (typeof aiDocsSettle === 'function') aiDocsSettle(); } catch (e) { console.error('documents apply', e); }
        ...
        try {
          if (window.ST && Array.isArray(rl) && !_unsent('rules')) ST.catRules = rl;
      ...
      try {
        ...
        /* The load found copies left by the old replace-all doubling and is showing one of
           each: save those areas now, so the server drops the rest (-448). */
